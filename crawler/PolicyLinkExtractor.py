...
# 
# CHANGES AND IMPROVEMENTS(v 3.0):
...
#    - Increased the number of parallel workers to 10 for improved performance.
...
# 9. Timeout Handling:
#    - Added a timeout logic to stop processing a domain if it takes more than 5 minutes, saving the links collected so far and moving to the next domain.
...
def process_domains(domains, country, max_depth=3):
    with ThreadPoolExecutor(max_workers=10) as executor:  # Increased workers to 10
        future_to_domain = {executor.submit(process_single_domain_with_timeout, domain, country, max_depth): domain for domain in domains}
...
def process_single_domain_with_timeout(domain, country, max_depth):
    start_time = datetime.now()
    timeout = timedelta(minutes=5)
    while datetime.now() - start_time < timeout:
        try:
            process_single_domain(domain, country, max_depth)
            break
        except Exception as e:
            print(f"Error processing domain {domain}: {e}")
    else:
        print(f"Timeout reached for domain {domain}, saving collected links.")
...
if __name__ == "__main__":
    ...
    korean_websites = korean_websites[:500]
